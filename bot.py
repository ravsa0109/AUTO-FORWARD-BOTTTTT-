...
@app.on_message(filters.command("listusers") & filters.user(OWNER_ID))
async def list_users(client, message):
    await message.reply("👥 Authorized Users:\n" + "\n".join(map(str, authorized_users)))

@app.on_message(filters.command("replacements") & filters.user(list(authorized_users)))
async def show_replacements(client, message):
    uid = message.from_user.id
    rep = replacements.get(uid, {})
    if not rep:
        return await message.reply("❌ No replacements set.")
    msg = "🔁 Your replacements:\n"
...
@app.on_message(filters.command("reset") & filters.user(list(authorized_users)))
async def reset_user_state(client, message):
    uid = message.from_user.id
    user_state.pop(uid, None)
    await message.reply("🔄 Forwarding state has been reset. Forward target message again.")
...
async def handle_forwarded(client, message: Message):
    ...
    if not message.forward_from_chat:
        return await message.reply("⚠️ Please forward a message from a channel (not a user).")
